...
import new_dataloading_D as Data
...
class FourDVarNetRunner:
    def __init__(self, dataloading="old", config=None):
        self.filename_chkpt = 'model-hinit=0-flagInit2-{epoch:02d}-{val_mse:.2f}-{tr_mse:.2f}'
        if config is None:
            import config 
        else:
    ...
    def train(self, ckpt_path='Results/model-hinit=0-flagInit2-epoch=61-val_mse=-1.79-tr_mse=-2.45.ckpt', **trainer_kwargs):
        """
        Train a model
        :param ckpt_path: (Optional) Checkpoint from which to resume
    ...
    def test(self, ckpt_path='Res_Update/model-hinit=0-flagInit2-epoch=18-val_mse=-1.74-tr_mse=-2.14.ckpt', dataloader="test", _mod=None, _trainer=None, **trainer_kwargs):
        """
        Test a model
        :param ckpt_path: (Optional) Checkpoint from which to resume
        :param dataloader: Dataloader on which to run the test Checkpoint from which to resume
