...
class LitModel(pl.LightningModule):
    def __init__(self,conf=HParam(),*args, **kwargs):
        ...
        self.hparams.iter_update     = [0,10,100,200,400,600]  # [0,2,4,6,9,15]
        self.hparams.nb_grad_update  = [10,10,10,15,20,20,20]  # [0,0,1,2,3,3]#[0,2,2,4,5,5]#
        self.hparams.lr_update       = [1e-4,1e-4,1e-4,1e-5,1e-5,1e-5,1e-6,1e-6,1e-7]
        ...
        #A revoir
        self.hparams.DimAE=5#150 avant
        self.hparams.dW = 3
        self.hparams.dW2=1
        self.hparams.sS = 2
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        out_debit = out
        return {'preds': out[:,:31,:].detach().cpu(),
                'p_diag' : out[:,31:,:].detach().cpu(),
                'init'      : input_init[:,:31,:].detach().cpu(),
    ...
    def compute_loss(self, batch, phase):
       ...
       # need to evaluate grad/backward during the evaluation and training phase for phi_r
        with torch.set_grad_enabled(True):
            ...
            '''
            #flagInit 2
            mean_state  =  outputs[:,:31,:]
            h_state = outputs[:,31:,:]
                        
            new_targets_GT =torch.zeros(targets_GT.shape[0], outputs.shape[1], targets_GT.shape[2]).to(device)
            new_targets_GT[:,:31,:] = targets_GT
            new_targets_GT[:,31:,:] = outputs[:,31:,:]  
            ...
            loss_c      = torch.mean((((mean_state - targets_GT)*torch.abs(h_state)*5)**2-2*torch.log(torch.abs(h_state)*5 +0.00001)))
                                                     
            loss_ML      = loss_c
            
            mat_c       = (((mean_state - targets_GT)*torch.abs(h_state)*5)**2-2*torch.log(torch.abs(h_state)*5 +0.00001))
            loss_ML_I   = torch.sum(mat_c * (1-masks))
            loss_ML_I   = torch.mul(1.0 / torch.sum(1-masks),loss_ML_I)
            
            loss_R      = torch.sum((outputs[:,:31,:] - targets_GT)**2* masks )
            loss_R      = torch.mul(1.0 / torch.sum(masks),loss_R)
            
            loss_I      = torch.sum((outputs[:,:31,:] - targets_GT)**2* (1. - masks) )
            loss_I      = torch.mul(1.0 / torch.sum(1.-masks),loss_I)
            
            #loss_AE_cov =  torch.mean((self.model.phi_r(out) - outputs)**2 )
            loss_AE_mu     = torch.mean((dyn_out[:,:31,:] - outputs[:,:31,:])**2 )
            loss_AE_GT_mu  = torch.mean((dyn_true[:,:31,:] - new_targets_GT[:,:31,:])**2 )
            loss_AE_cov    = torch.mean((dyn_out[:,31:,:] - outputs[:,31:,:])**2 )
            loss_AE_GT_cov = torch.mean((dyn_true[:,31:,:] - new_targets_GT[:,31:,:])**2 )
            
            loss_squared = loss_I+loss_R
            
                               
            
            loss         = 0.01* self.hparams.alpha[0] * loss_ML+ 0.1*loss_ML_I +loss_squared+ 0.1 * self.hparams.alpha[1] * ( loss_AE_mu + loss_AE_GT_mu )+0.1*(loss_AE_cov+loss_AE_GT_cov)
