...
import dataloading_stochastic as Data
...
class FourDVarNetRunner:
    def __init__(self, dataloading="old", config=None):
        self.filename_chkpt = 'modelsto-hinit=0-flagInit2-{epoch:02d}-{val_mse:.2f}-{tr_mse:.2f}'
        if config is None:
            import config 
        else:
    ...
    def test(self, ckpt_path='Res_Update/modelsto-hinit=0-flagInit2-epoch=22-val_mse=-8.33-tr_mse=-8.89.ckpt', dataloader="test", _mod=None, _trainer=None, **trainer_kwargs):
        """
        Test a model
        :param ckpt_path: (Optional) Checkpoint from which to resume
        :param dataloader: Dataloader on which to run the test Checkpoint from which to resume
