...
def save_NetCDF(saved_path1, preds, p_diag,p_outdiag, init, target,mask ):    
        N = preds.shape[0]
        print(N)
        dT = preds.shape[1]
        dX = preds.shape[2]
        space = np.linspace(1,dX,dX)
        time = np.linspace(1,dT,dT)
        Nb = np.linspace(1,N,N)
        print(Nb)
        xrdata = xr.Dataset( 
                        data_vars=dict(preds =(["Nb_samples", "time","x"],preds),
                                       p_diag =(["Nb_samples", "time","x"],p_diag),
                                       p_outdiag =(["Nb_samples", "time","x"],p_outdiag),
                                       inits =(["Nb_samples", "time","x"],init),
                                       targets=(["Nb_samples", "time","x"],target),
                                       masks = (["Nb_samples", "time","x"],mask),
                                      ),
                            coords = dict(Nb_samples = (["Nb_samples"],Nb),
                                          time = time,
                                          space =(["x"],space),
                                         ),
                                       )
...
def save_NetCDF2(saved_path1, preds, p_diag, init, target,mask ):    
        N = preds.shape[0]
        print(N)
        dT = preds.shape[1]
        dX = preds.shape[2]
